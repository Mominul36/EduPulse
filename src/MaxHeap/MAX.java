 ...
 * @author Raven
 */
public class MAX extends javax.swing.JPanel {
           ...
           try {
            int data = Integer.parseInt(txtinsert.getText());
              this.max.insert(data);
               print(max.getarray(),max.getsize());
              TreeBB myTree = new TreeBB();
             // simulador.setroot(null);
              createmax cr = new createmax(max.getarray(),max.getsize());
              myTree.setRoot(cr.getRoot());
                complementos(myTree);
                removeorder();
 ...
 if(evt.getKeyCode()==KeyEvent.VK_LEFT)
    ...
    public void complementos(TreeBB myTree){
        this.repintarArbol(myTree);
    }
    private void repintarArbol(TreeBB myTree) {
        ...
        this.internalframe.add(myTree.getdrawing(), BorderLayout.CENTER);
    }
