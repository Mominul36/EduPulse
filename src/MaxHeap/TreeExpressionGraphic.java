...
public class TreeExpressionGraphic extends JPanel 
{
    ...
    private Dimension calculateSizeSutree(Node n) 
    {
          ...
          Dimension ld = calculateSizeSutree(n.left);
          Dimension rd = calculateSizeSutree(n.right);
    ...
    private void calculatePosition(Node n, int left, int right, int top) 
    {
      ...
      int width = fm.stringWidth(n.data+"");
 
      positionNodes.put(n,new Rectangle(center - width/2 - 3, top, width + 6, fm.getHeight()));
      
      calculatePosition(n.left, Integer.MAX_VALUE, center - child2child/2, top + fm.getHeight() + parent2child);
      calculatePosition(n.right, center + child2child/2, Integer.MAX_VALUE, top + fm.getHeight() + parent2child);
    }
    ...
    private void drawTree(Graphics2D g, Node n, int puntox, int puntoy, int yoffs) 
    {
     ...
     g.draw(r);
     g.drawString(n.data+"", r.x + 3, r.y + yoffs);
   
     if (puntox != Integer.MAX_VALUE)
       
     g.drawLine(puntox, puntoy, (int)(r.x + r.width/2), r.y);
     
     drawTree(g, n.left, (int)(r.x + r.width/2), r.y + r.height, yoffs);
     drawTree(g, n.right, (int)(r.x + r.width/2), r.y + r.height, yoffs);
